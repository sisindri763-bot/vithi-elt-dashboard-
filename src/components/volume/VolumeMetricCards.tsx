"use client"

import { Database, BarChart3, Activity, Play } from "lucide-react"
import type { VolumeResponse } from "@/hooks/useVolumeData"

interface MetricCardProps {
  icon: React.ReactNode
  iconBgColor: string
  title: string
  value: string
  changeText?: string
  changeColor?: string
  subtitle: string
  tone?: string
}

function MetricCard({
  icon,
  iconBgColor,
  title,
  value,
  changeText,
  changeColor,
  subtitle,
  tone,
}: MetricCardProps) {
  const isWarning = tone === "warn"
  const cardClassName = isWarning 
    ? "bg-orange-50 border-2 border-orange-200 hover:border-orange-300" 
    : "bg-white border border-gray-200 hover:shadow-md"
    
  return (
    <div className={`rounded-xl px-4 py-3 shadow-sm transition-all ${cardClassName}`}>
      {/* Warning indicator */}
      {isWarning && (
        <div className="flex items-center gap-1 mb-1">
          <span className="text-orange-500 text-xs">⚠️</span>
          <span className="text-orange-700 text-xs font-medium">Warning</span>
        </div>
      )}
      
      {/* Header */}
      <div className="flex items-center gap-2 mb-2">
        <div className={`w-7 h-7 rounded-lg ${iconBgColor} flex items-center justify-center`}>
          {icon}
        </div>
        <span className="text-xs font-medium text-gray-500">{title}</span>
      </div>

      {/* Value */}
      <div className="flex items-baseline gap-1.5 mb-2">
        <span className="text-2xl font-bold text-gray-900">{value}</span>
        {changeText && (
          <span className={`text-sm font-semibold ${changeColor}`}>
            {changeText}
          </span>
        )}
      </div>

      {/* Subtitle */}
      <p className="text-[11px] text-gray-400">{subtitle}</p>
    </div>
  )
}

interface VolumeMetricCardsProps {
  data: VolumeResponse | null
  loading: boolean
  error: string | null
}

export function VolumeMetricCards({ data, loading, error }: VolumeMetricCardsProps) {
  if (loading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="bg-white rounded-xl border border-gray-200 px-4 py-3 animate-pulse">
            <div className="h-7 bg-gray-100 rounded mb-2"></div>
            <div className="h-6 bg-gray-100 rounded mb-2"></div>
            <div className="h-3 bg-gray-100 rounded"></div>
          </div>
        ))}
      </div>
    )
  }

  if (error || !data) {
    return (
      <div className="bg-rose-50 border border-rose-200 rounded-xl p-3 text-sm text-rose-700">
        Error loading volume metrics: {error || "Unknown error"}
      </div>
    )
  }

  const kpis = data.kpis

  const getChangeColor = (tone: string, delta: number | null) => {
    if (tone === "bad") return "text-rose-600"
    if (tone === "warn") return "text-orange-600"
    if (tone === "ok") return "text-emerald-600"
    if (delta !== null && delta !== 0) {
      return delta > 0 ? "text-emerald-600" : "text-rose-600"
    }
    return "text-gray-400"
  }

  const getChangeText = (kpi: any) => {
    if (kpi.delta !== null && kpi.delta !== undefined) {
      const sign = kpi.delta >= 0 ? "+" : ""
      return `${sign}${kpi.delta} ${kpi.delta_label || ""}`
    }
    return kpi.delta_label || ""
  }

  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
      {kpis.map((kpi) => {
        const changeColor = getChangeColor(kpi.tone, kpi.delta)
        const changeText = getChangeText(kpi)
        
        // Dynamic icon selection with tone-based styling
        let icon = <Database size={16} />
        let iconBg = "bg-blue-50 border border-blue-100"
        
        switch (kpi.id) {
          case "data_received":
            icon = <Database size={16} className="text-blue-600" />
            iconBg = "bg-blue-50 border border-blue-100"
            break
          case "records_received":
            icon = <BarChart3 size={16} className="text-emerald-600" />
            iconBg = "bg-emerald-50 border border-emerald-100"
            break
          case "pipelines_active":
            // Change styling based on tone
            if (kpi.tone === "warn") {
              icon = <Activity size={16} className="text-orange-600" />
              iconBg = "bg-orange-50 border border-orange-100"
            } else if (kpi.tone === "bad") {
              icon = <Activity size={16} className="text-red-600" />
              iconBg = "bg-red-50 border border-red-100"
            } else {
              icon = <Activity size={16} className="text-purple-600" />
              iconBg = "bg-purple-50 border border-purple-100"
            }
            break
          case "runs":
            icon = <Play size={16} className="text-orange-600" />
            iconBg = "bg-orange-50 border border-orange-100"
            break
          default:
            icon = <Database size={16} className="text-gray-600" />
            iconBg = "bg-gray-50 border border-gray-100"
        }

        return (
          <MetricCard
            key={kpi.id}
            icon={icon}
            iconBgColor={iconBg}
            title={kpi.title}
            value={kpi.display}
            changeText={changeText}
            changeColor={changeColor}
            subtitle={kpi.available ? "Last 30 days" : "Not available"}
            tone={kpi.tone}
          />
        )
      })}
    </div>
  )
}